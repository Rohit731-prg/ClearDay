...
from app.Database.Schema.UserSchme import LoginSchema
...
async def login(email: str, password: str) -> dict:
    try:
        ...
        compair = compairPassword(password, user["password"])
        if not compair:
            raise HTTPException(status_code=400, detail="Invalid password")
        userDeyails = LoginSchema(user)
        token = generaytToken(user)
        return { "token": token, "user": userDeyails, "message": "User logged in successfully" }
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))